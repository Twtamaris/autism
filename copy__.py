...
def save_beat(file_path, beat_name, beats, bpm, clicked, saved_beats):
    """Save the current beat configuration to a file."""
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            ...
            # Save beat logic
            if save_menu and saving_button.collidepoint(event.pos):
                saved_beats = save_beat('saved_beats.txt', beat_name, beats, bpm, clicked, saved_beats)
                save_menu = False
                playing = True
            
            # Load beat logic
            if load_menu and loading_button.collidepoint(event.pos):
                if 0 <= index < len(saved_beats):
        ...
        # Handle text input and backspace for beat name
        beat_name = handle_text_input(event, beat_name, typing)
