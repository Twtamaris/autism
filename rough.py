    ...
    for i in range(beats):
        for j in range(2, instruments):
            ...
            rect = pygame.draw.rect(
                screen,
                color,
                [i * rect_width + 205, (j * 100) + 5, rect_width - 10, 90],
                0,
                3,
            )
            ...
            pygame.draw.rect(
                screen,
                gold,
                [i * rect_width + 200, j * 100, rect_width, rect_height],
                5,
                5,
            )
            pygame.draw.rect(
            ...
            if k < len(list_pic):
                ...
                image = pygame.transform.scale(
                    list_pic[k], (rect_width - 20, rect_height - 20)
                )  # Adjust image size
                screen.blit(
                    image, (211 + i * rect_width, (j * 100) + 10)
...
def draw_save_menu(beat_name, typing):
    ...
    menu_text = label_font.render("SAVE MENU: Enter a Name for this Sound", True, white)
...
def draw_load_menu(index):
    ...
    loaded_bpm = 0
    pygame.draw.rect(screen, black, [0, 0, WIDTH, HEIGHT])
    menu_text = label_font.render("LOAD MENU: Select a Sound to load in", True, white)
    screen.blit(menu_text, (400, 40))
    exit_btn = pygame.draw.rect(
        screen, gray, [WIDTH - 200, HEIGHT - 100, 180, 90], 0, 5
    )
...
class Button:
    ...
    def play_pause_button(self, playing):
        play_pause = pygame.draw.rect(
            self.screen, (255, 102, 255), [50, self.HEIGHT - 150, 200, 100], 0, 5
        )
        play_text = self.label_font.render("Play/Pause", True, self.white)
        self.screen.blit(play_text, (70, self.HEIGHT - 130))

        if playing:
            play_text2 = self.medium_font.render(
                "Playing", True, (50, 50, 50)
            )  # dark gray
        else:
            pygame.draw.rect(
                self.screen, (102, 0, 204), [50, self.HEIGHT - 150, 200, 100], 0, 5
            )
            play_text = self.label_font.render("Play/Pause", True, self.white)
            self.screen.blit(play_text, (70, self.HEIGHT - 130))
            play_text2 = self.medium_font.render(
                "Paused", True, (50, 50, 50)
            )  # dark gray
    ...
    def speed_button(self):
        bpm_rect = pygame.draw.rect(
            self.screen, (0, 255, 255), [300, self.HEIGHT - 150, 200, 100], 5, 5
        )
        pygame.draw.rect(
            self.screen, (0, 255, 255), [300, self.HEIGHT - 150, 200, 100 / 2], 5, 5
        )
        ...
        bpm_add_rect = pygame.draw.rect(
            self.screen, (102, 255, 102), [510, self.HEIGHT - 150, 48, 48], 0, 5
        )
        bpm_sub_rect = pygame.draw.rect(
            self.screen, (204, 0, 0), [510, self.HEIGHT - 100, 48, 48], 0, 5
        )
    ...
    def board_resize_button(self):
        beats_rect = pygame.draw.rect(
            self.screen, self.gold, [600, self.HEIGHT - 150, 200, 100], 3, 5
        )
        ...
        beats_add_rect = pygame.draw.rect(
            self.screen, self.gray, [810, self.HEIGHT - 150, 48, 48], 0, 5
        )
        beats_sub_rect = pygame.draw.rect(
            self.screen, self.gray, [810, self.HEIGHT - 100, 48, 48], 0, 5
        )
    ...
    def clear_board_button(self):
        if self.green_is_on == 1:
            clear = pygame.draw.rect(
                self.screen, (102, 0, 204), [1150, self.HEIGHT - 150, 200, 100], 0, 5
            )
        else:
            clear = pygame.draw.rect(
                self.screen, (255, 102, 255), [1150, self.HEIGHT - 150, 200, 100], 0, 5
            )
    ...
    def load_and_save_button(self):
        save_button = pygame.draw.rect(
            self.screen, self.gray, [900, self.HEIGHT - 150, 200, 48], 0, 5
        )
        pygame.draw.rect(
            self.screen, (0, 255, 255), [900, self.HEIGHT - 150, 200, 48], 3, 5
        )
        ...
        load_button = pygame.draw.rect(
            self.screen, self.gray, [900, self.HEIGHT - 98, 200, 48], 0, 5
        )
        load_text = self.label_font.render("Load Sound", True, self.white)
        pygame.draw.rect(
            self.screen, (0, 255, 255), [900, self.HEIGHT - 98, 200, 48], 3, 5
        )
    ...
    def initial_type_bar(self, green_is_on, new_messages_list):
        ...
        if green_is_on == 0:
            ...
            text_ = self.label_font.render(
                "Press Key to Start...", True, (207, 201, 201)
            )
...
def adjust_beats(
    beats_add_rect, beats_sub_rect, event_pos, beats, clicked, instruments
):
...
while run:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            # Play/Pause Button
            playing, active_beat, active_length = toggle_play_pause(
                play_pause, event.pos, playing, active_beat, active_length
            )
            ...
            # Adjust Beats
            beats, clicked = adjust_beats(
                beats_add_rect, beats_sub_rect, event.pos, beats, clicked, instruments
            )
            ...
            # Toggle Instruments
            active_list = handle_instrument_click(
                instrument_rects, event.pos, active_list
            )
    ...
    if playing:
        ...
        for i in range(beats):  # Assuming `beats` is the total number of beats
            ...
            # If all columns in the row are deselected (-1), use a shorter beat length
            if sum(columns_selected) != -4:
                new_list.append(40)  # Shorter beat length if no columns selected
            else:
                new_list.append(0)  # Normal beat length when columns are selected
